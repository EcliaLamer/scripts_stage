...
# ATTENTION : ce script est ecrit pour une machine a 40 coeurs
print("ici on cree nos datas proche bord corrigees")
number_cores = 40
rng_seed = 7521
...
def worker(id_process, nb_process, rng_seed):
    ...
    for simu in range(250):
        ...
        path_output_folder = 'output/output_centre_bord_2/data'
        # path to a folder where to save the start_vertice
        path_output_folder_start = 'output/output_centre_bord_2/start'
        ...
        start_vertice = None
        while start_vertice is None:
            vertex = tuple(np.random.randint(100,size=2))
            if vertex[0] not in range(1,99) or vertex[1] not in range(1,99):
                start_vertice = vertex
        if start_vertice[1]==99:
            neig_vertice = (start_vertice[0], start_vertice[1] - 1)
        else :
            neig_vertice = (start_vertice[0], start_vertice[1] + 1)
        list_start_vertices = [start_vertice, neig_vertice]
        #print(graph.dict_cell_id_to_ind[start_vertice])
