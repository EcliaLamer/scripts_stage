...
def worker(id_process, nb_process, rng_seed):
    ...
    for simu in range(1000):
        ...
        path_output_folder = 'output/example_first_K15/data'
        # path to a folder where to save the start_vertice
        path_output_folder_start = 'output/example_first_K15/start'
        ...
        start_vertice = tuple(np.random.randint(100,size=2))
        if start_vertice[1]==100:
    ...
    np.save(path_output_folder_start +'_process' +str(id_process) +'.npy', arr_mid_start_vertices)
