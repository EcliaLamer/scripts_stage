...
output_path = 'output_for_nn_with_all_resources_and_without_null/'
...
model.save('output_for_nn_with_all_resources_and_without_null/my_model_with_resources')
...
# On cherche les indices des cartes nulles

list_ind_map_null = []
for i, carte in enumerate(test_dataset):
    if np.sum(carte.flatten()) == 0.0:
        list_ind_map_null.append(i)

print("nombre de cartes nulles :" + str(len(list_ind_map_null)))

test_dataset_without_null = np.delete(test_dataset, list_ind_map_null, 0)
test_target_without_null = np.delete(test_target, list_ind_map_null, 0)
test_resources_without_null = np.delete(test_resources, list_ind_map_null, 0)

np.savez_compressed(output_path + 'test_dataset_without_null', a=test_dataset_without_null)
np.savez_compressed(output_path + 'test_target_without_null', a=test_target_without_null)
np.savez_compressed(output_path + 'test_resources_without_null', a=test_resources_without_null)

 
print("Testing Dataset Shapes: " + str(test_dataset_without_null.shape) + ", " + str(test_target_without_null.shape) + "," + str(test_resources_without_null.shape))

# On teste notre NN

#     # loss and accuracy test
score_without_null = model.evaluate({'input_inf': test_dataset_without_null, 'input_ressource': test_resources_without_null}, {'output_start': test_target_without_null}, verbose=2)
    #test_acc = score[1]
    #test_loss = score[0]
np.savez_compressed(output_path + 'score_without_null', a=score_without_null)
print('Le score du model est ', score_without_null)


    # predictions

pred_without_null = model.predict({'input_inf': test_dataset_without_null, 'input_ressource': test_resources_without_null})

np.savez_compressed(output_path + 'pred_without_null', a= pred_without_null)
