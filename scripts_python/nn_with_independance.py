...
# ----------------------------------------------------------------
# parametres
nb_repetition_train_nn = 10
base_seed = 1200

# create folder if needed
base_path_folder_output = 'output_indepence_K10'
if os.path.exists(base_path_folder_output):
    raise ValueError("Already a Folder at " + base_path_folder_output)
os.mkdir(base_path_folder_output)
# ----------------------------------------------------------------

print("Ici on entraine", nb_repetition_train_nn, "nn avec un k de 10")


start_time = time.time()

for k in range(nb_repetition_train_nn):
    path_folder_output = base_path_folder_output + '/' + str(k + 1)
    os.mkdir(path_folder_output)

    # On importe les datas brutes
    # on recup lelien des donnees de calcul canada
    name_data_list = list(glob.glob('data_indepence/K10/' + str(k + 1) + '/data*.npy'))
    name_target_list = list(glob.glob('data_indepence/K10/' + str(k + 1) + '/start*.npy'))
...
# test avec plus grand filtre

path_folder_output = base_path_folder_output + '/test_bigger_filter' 
os.mkdir(path_folder_output)

# On importe les datas brutes
# on recup lelien des donnees de calcul canada
name_data_list = list(glob.glob('data_indepence/K10/1/data*.npy'))
name_target_list = list(glob.glob('data_indepence/K10/1/start*.npy'))
...
# Première couche convolutive
model.add(Conv2D(32, (5, 5), padding='same', input_shape = (100, 100, 1), activation = 'relu'))
model.add(MaxPooling2D(pool_size=(2, 2)))
